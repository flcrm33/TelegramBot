...
async def make_changes_command(message: types.Message):
    ...
    await bot.send_message(message.from_user.id, 'Чого тобі хазяїн???', reply_markup=admin_kb.button_case_admin)
...
# Ловимо першу відповідь і пишемо словник
...
async def load_photo(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await FSMAdmin.next()
        await message.reply('Тепер вводь назву')
...
async def load_name(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await FSMAdmin.next()
        await message.reply('Вводь опис')
...
async def load_description(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await FSMAdmin.next()
        await message.reply('А зараз ціну')
...
# Інлайн кнопка ВИДАЛИТИ - хендлери
